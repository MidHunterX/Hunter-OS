...
preference. Even in the absence of a display server, do 95% of work in TTY
comfortability without disrupting the workflow.
...
|===
| Key   | Description
| i     | go inside, enter inside selected
| o     | step out, stay outside and open with, step outside selected
| <C-i> | history forward, zoom in
| <C-o> | history backward, zoom out
| <C-u> | scroll up
| <C-d> | scroll down
| ?     | help, keybindings
| /     | search mode
| x     | exit from sub-features (like menus, options etc.) or quit if none are present
| q     | quit
|===
