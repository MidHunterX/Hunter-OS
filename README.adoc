...
| sway-audio-idle-inhibit-git | Prevents idle on audio       | aur
|===
...
NOTE: Firefox supports the Wayland inhibit protocol, but it attempts to use the DBus interfaces first. It never reaches its Wayland inhibit-idle protocol fallback because the portal indicates success, even if there's no backend https://github.com/flatpak/xdg-desktop-portal-gtk/issues/465[#465]. `sway-audio-idle-inhibit-git` Prevents swayidle/hypridle from sleeping while any application is outputting or receiving audio. This only works for Pulseaudio / Pipewire Pulse.
...
image:Mid_Hunter/scripts/.assets/auto_adaptation.jpg[link=automata/iris/]
