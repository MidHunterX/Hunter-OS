= Hunter OS - pass:[<span title="Wánzhěng">完整</span>]
...
| link:.config/quickshell/[⚡ Quickshell]
| Status Bar
| image:.config/quickshell/.assets/quickshell.png[,128,link=.config/quickshell/]
...
|===
| 🔴 | Required Interconnected Dependency
| 🟡 | Optional Interconnected Dependency
|===
...
fish --> ls & vifm & cd & git & cat & prompt
cd --> ls --> nf
prompt & fish --$VIFM_MODE--> vifm
fish --$editor--> nvim
vifm --> ls & nf
vifm --syntax highlighting--> cat
...
IMPORTANT: Color generators like matugen are so pervasive, it becomes a dependency for app config colors unless there's a way to set fallback colors on app configs. This generally makes the configs not plug-n-play friendly but, the main focus of this section is to make every configs independent (matugen Optional) and plug-n-play friendly again.

|===
| 🟢 | matugen Optional (Fallback Colors)     | [12/16]
| 🟡 | matugen Optional (Fallback to Pink)    | [02/16]
| 🔴 | matugen Required (No Fallback Colors)  | [02/16]
|===
...
expression(<a href='{gh-expression}'>Expression</a>)
vifm(<a href='{gh-repo-master}/.config/vifm'>ViFM</a>)
iris(<a href='{gh-repo-master}/agents'>Iris</a>)
...
gen --> power[🔴 wlogout]
gen --> waybar[🟢 <del>waybar</del> QuickShell]
gen --> vis[🔴 cava]
...
- [.line-through]#Waybar &# Wlogout: There's no way to conditionally source/implement fallback in GTK-CSS. Only way is to copy a generated file initially.
...
| wlctl-bin                 | TUI for networkmanager, Vim keys   | aur
| speedometer               | TUI Network Speed Monitor          | aur
|===

* `firefox-developer-edition` allows you to load unverified extensions and bypass stringent security systems imposed by vanilla firefox. This is needed for doing the Movie Streaming Buffer Preloading hack.
...
| adw-gtk-theme               | libadwaita theme             | pacman
...
| quickshell                  | Qt GUI Shell                 | pacman
...
.Installing Dark Theme (adw-gtk-theme)
[source,bash]
----
...
Track terminal emulator support for Unicode's "Symbols for Legacy Computing" (U+1FB00-U+1FBFF) and its supplement block (U+1CC00-U+1CEBF).

https://notcurses.com/::
A library facilitating complex TUIs on modern terminal emulators, supporting vivid colors, multimedia, threads, and Unicode to the maximum degree possible.
====
...
[discrete]
.Core Philosophies
- Eliminating friction between intent and execution
...
- You should not be held back by the tools that you use. The tools should be customizable, scriptable or at least molded to your needs.
- Expanding the Self by removing the boundaries of where the Self ends and tools begin.
...
link:agents/stasis[Auto Idle]: The device will shutdown itself when it's left
...
link:agents/iris[Adaptive Window Contrast]: An adaptive
...
image:agents/.assets/auto_adaptation.jpg[link=agents/iris/]
...
.Copy en_HUNTER Locale and Compile
[source,bash]
----
sudo cp ~/.config/LOCALE/en_HUNTER /usr/share/i18n/locales/
sudo localedef -i en_HUNTER -f UTF-8 en_HUNTER
----

.Add entry to /etc/locale.gen
[source,bash]
----
...
[source,bash]
----
LANG=en_HUNTER.UTF-8
----

Reboot your computer. Run `cal` to see the new change. Months should be now in
proper Roman month names adapted to the Gregorian calendar: Primilis,
Secundilis, Tertilis, Quartilis, Quintilis, Sextilis, September, October,
November, December, Undecember and Duodecember.

Uses -ilis for months 1–6 (since those were the original numbered months in the
old Roman calendar), and -ber for months 7–12 (since that was the suffix they
historically chose for the later months).

Now you can properly tell the month number just by the name and vice-versa
unlike the "modern" inconsistent slop.
====
...
=== Fix Video Buffering issues on streaming Movies
Install link:https://github.com/Andrews54757/FastStream[FastStream] in
`firefox-developer-edition`. It takes over the video streaming player;
overriding by a custom player that aggressively pre-buffers the rest of the
video/movie in the background while you watch it. Supports HLS streams (.m3u8)
and DASH streams (.mpd). Video player will now show buffer download speed,
comprehensive subtitle UI supporting search and download.

=== Fix Low Volume Dialogue + Blown Out Sound FX in Movies
Install `easyeffects`, `lsp-plugins-lv2` and `calf`. Open `easyeffects` and add
the *Autogain* effect. It will automatically detect and calibrate volume gain
in real-time. After this, just open up `easyeffects` whenever you are watching
any movies.

=== AutoEQ to another IEM

==== Step 1: Parametric Equalization
* Open link:https://squig.link[squig.link]
* Disable all targets from the active graph window.
* Select your IEM from `models` section.
* Add the IEM you want to try with `+` button.
* Download the graph of the IEM you want to try (Target).
* Go to `equalizer` section
* Parametric Equalizer = Your IEM
* AutoEQ Range = 20-6000 hz (we shouldn't AutoEQ treble as treble graphs are not accurate)
* Hit `AutoEQ` button and click `Export` for Linux or `Export Graphic EQ` for Android.

==== Step 2: Loading Frequency Response Graph
* Open `easyeffects`
* Add *Equalizer* effect > More Actions > Import APO
* Load the previously downloaded *ParametricEQ* file.

On Android Smartphone, you can use Wavelet app to load the downloaded *GraphicEQ* file.
