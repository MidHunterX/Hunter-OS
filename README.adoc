...
| ttf-twemoji             | Google Font for Unicode Support | pacman
|===

.To enable twemoji (especially on terminal emulators):
[source,bash]
----
sudo ln -s /usr/share/fontconfig/conf.avail/75-twemoji.conf /etc/fonts/conf.d/
----
Alternative Emoji TTF: ttf-joypixels - Premium high quality emoji fonts
