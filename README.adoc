...
expression & vifm --> ai[ai_theme.sh]
ai --> gen[matugen] & hypr[🟢 hyprland]
gen --> hypr --> 1_colors_fallback.conf --colors/2_colors.conf--> override_sourcing
...
gen --> top[🟢 btop] --colors.theme--> manual_selection
gen --> app[🟢 fuzzel] --colors.ini--> default_fallback
gen --> power[🔴 wlogout]
gen --> bar[🔴 waybar]
gen --> vis[🔴 cava]
...
- cava: No option for theme fallback in config as it is doing an in-built parsing. The best way would be to find/create a better wrapper using cava as backend.

Solution: Use custom made status bar, logout screen etc. with a custom GUI shell like QuickShell, AGS or EWW.
====
...
subgraph Development Area
	tmux(<a href='https://github.com/MidHunterX/Hunter-OS/tree/master/.config/tmux/'>TMUX</a>)
	run(<a href='https://github.com/MidHunterX/Project-Launcher'>Project Launcher</a>)
	git(<a href='https://github.com/MidHunterX/Hunter-OS/tree/master/.config/lazygit/'>Lazygit</a>)
end
...
hypr -.Cursor Mode (Meta+e c).-> cursor
nvim -.Immediate Jump (f).-> cursor
nvim -.Local Jump (s).-> cursor
nvim -.Global Jump (leader sf).-> cursor
hypr--(Meta+e f)--> firefox
firefox(<a href='https://github.com/MidHunterX/Hunter-OS/tree/master/.mozilla/'>Firefox</a>)
...
WARNING: Color generators like matugen are so pervasive, it becomes a dependency for app config colors unless there's a way to set fallback colors on app configs. This makes the configs non-plug-n-play for other systems until all variables are replaced with hex colorcodes. Another mitigation technique is to generate the whole config from the generator itself (Not a fan of this technique).
