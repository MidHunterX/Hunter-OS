...
Sometimes *dhcpcd* will generate configuration which also includes DNS nameservers from your ISP in `/etc/resolv.conf`. We gotta stop that.
...
Now, set all the nameservers you want to use instead.
...
.💡 Using other DNS Resolvers?
[%collapsible]
====
[discrete]
If you use systemd-resolved or NetworkManager, they might override this file. In that case, you can set your static nameservers inside their configs

* /etc/systemd/resolved.conf for systemd-resolved
* /etc/NetworkManager/conf.d/ for NetworkManager
====
