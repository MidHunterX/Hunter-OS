...
Install link:https://github.com/Andrews54757/FastStream[FastStream] in `firefox-developer-edition`. It takes over the video streaming player; overriding by a custom player that aggressively pre-buffers the rest of the video/movie in the background while you watch it. Supports HLS streams (.m3u8) and DASH streams (.mpd). Video player will now show buffer download speed, comprehensive subtitle UI supporting search and download.

=== Fix Low Volume Dialogue + Blown Out Sound FX in Movies
Install `easyeffects`, `lsp-plugins-lv2` and `calf`. Open `easyeffects` and add the *Autogain* effect. It will automatically detect and calibrate volume gain in real-time. After this, just open up `easyeffects` whenever you are watching any movies.
