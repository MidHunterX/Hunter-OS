...
WARNING: Color generators like matugen are pervasive that it becomes a core dependency for app config colors unless there's a way to do fallback color on app configs. This makes the configs non-plug-n-play for other systems until all variables are replaced with hex colorcodes. TLDR: colors don't work without matugen on some apps.
