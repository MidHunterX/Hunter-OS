...
WARNING: Color generators like matugen are so pervasive, it becomes a dependency for app
config colors unless there's a way to set fallback colors on app configs. This
makes the configs non-plug-n-play for other systems until all variables are
replaced with hex colorcodes. Another mitigation technique is to generate the
whole config from the generator itself (Not a fan of this technique).

[discrete]
==== 🎨 Diligently Directed Dynamic Design Definitions

`matugen` is used to generate dynamic Material You colors. Currently, each
config is in the process of getting fallback colors for portability.

|===
| 🔴 | matugen Required as of now
...
gen --> hypr --> 1_colors_fallback.conf --> 2_colors.conf
gen --> tmux[🟢 tmux] --> colors_fallback.conf --> colors.conf
gen --> bar[🔴 waybar]
gen --> vis[🔴 cava]
gen --> notif[🔴 dunst]
gen --> term[🔴 kitty]
...
- Waybar: There's no way to conditionally source/implement fallback in GTK-CSS. Only way is to copy a generated file initially.
